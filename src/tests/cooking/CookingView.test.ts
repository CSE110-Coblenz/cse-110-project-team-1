...
vi.mock('konva', () => ({
	default: {
		...
		Rect: class {
			...
			stroke(val?: string | null) {
				if (val !== undefined) {
					this.attrs.stroke = val;
					return this;
				}
		...
		Group: class {
			...
			on(_event: string, _handler: any) {}
			getClientRect() {
				return { x: this.attrs.x ?? 0, y: this.attrs.y ?? 0, width: 64, height: 64 };
			}
		},
		Tween: class {
...
describe('CookingView', () => {
	...
	it('initializes minimal view elements', () => {
		...
		expect(
			mockStages.find((s: any) => s.container === 'progress-konva-container'),
		).toBeTruthy();
	});
