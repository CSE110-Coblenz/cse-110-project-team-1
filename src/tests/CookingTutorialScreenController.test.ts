...
vi.mock('konva', () => {
  ...
  return {
    default: {
      Image: function() { return mockImageInstance; },
      Group: function() { return mockGroupInstance; },
    },
...
describe('CookingTutorialScreenController', () => {
  ...
  it('clicking tutorial image sets tutorialSeen and routes to cooking', async () => {
    ...
    const controller = new CookingTutorialScreenController(screenSwitcher as any, species as any, 2);
    ...
    expect(screenSwitcher.switchToScreen).toHaveBeenCalledWith({ type: 'cooking', species, nextLevel: 2 });
  });
});
