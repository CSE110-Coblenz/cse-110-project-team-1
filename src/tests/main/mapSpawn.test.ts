...
describe('MapModel spawn safety', () => {
    it('does not place walls in player spawn area across many random maps', () => {
        ...
        for (let i = 0; i < runs; i++) {
            const model = new MapModel({ width, height, wallCount: 50, wallMinWidth: minW, wallMaxWidth: maxW });
            const spawnX = Math.floor(width / 2);
            const spawnY = Math.floor(height / 2);
            const margin = maxW + 50;
            const spawnRect = { minX: spawnX - margin, minY: spawnY - margin, maxX: spawnX + margin, maxY: spawnY + margin };
            ...
            for (const wall of walls) {
                ...
                const overlaps = !(bmaxX < spawnRect.minX || bminX > spawnRect.maxX || bmaxY < spawnRect.minY || bminY > spawnRect.maxY);
