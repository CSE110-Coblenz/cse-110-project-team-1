...
export class GameScreenView implements View {
  ...
  constructor(stageWidth = 800, stageHeight = 600) {
    ...
    this.healthLabel = new Konva.Text({ x: 0, y: 0, text: `HP: 100%`, fontSize: 14, fill: '#ffffff', fontStyle: 'bold' });
  ...
  resize(stageWidth: number, stageHeight: number): void {
    ...
    this.hudPanel.cornerRadius(Math.round(Math.min(this.hudPanel.width(), this.hudPanel.height()) * 0.07));
  }
