...
export class PlayerController {
    ...
    constructor(model: PlayerModel, view: PlayerView, mapModel: MapModel, mapController: MapController) {
        this.model = model;
        this.view = view;
    ...
    private tryMove(dx: number, dy: number) {
        ...
        for (const [ox, oy] of offsets) {
            if (this.mapModel.isPointInsideWall(Math.floor(nx + ox), Math.floor(ny + oy))) return false;
        }
