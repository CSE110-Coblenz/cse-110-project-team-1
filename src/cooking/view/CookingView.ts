...
export class CookingView {
	...
	showGameOver(finalScore: number, onClose?: () => void, boostInfo?: { boost: number; newSpeed: number; baseSpeed: number }): void {
		...
		if (!overlay) {
			...
			panel.innerHTML = boostInfo
				? `
				<div style="font-size: 22px; font-weight: 700; color: #111827; margin-bottom: 12px;">Game Over</div>
				<div style="font-size: 16px; color: #4b5563; margin-bottom: 4px;">You've earned a speed boost!</div>
				<div style="font-size: 18px; color: #111827; margin-bottom: 12px;">Your next speed will be <strong style="color: #3b82f6;">${boostInfo.newSpeed.toFixed(0)}</strong></div>
